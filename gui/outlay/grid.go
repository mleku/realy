...
func (a *AxisPosition) normalize(gtx Gx, axis Axis, elements int, dimensioner Dimensioner) {
	if a.First < 0 {
		a.First = 0
	}
	if a.First > elements {
...
func (a *AxisPosition) computePosition(gtx Gx, axis Axis, elements, maxPx int, dimensioner Dimensioner) (pixelsUsed int) {
	constraint := axis.Convert(gtx.Constraints.Max).X
...
func (a *AxisPosition) update(gtx Gx, axis Axis, elements, maxPx int, dimensioner Dimensioner) {
...
func (g *Grid) drawRow(gtx Gx, row, rowHeight int, dimensioner Dimensioner, cellFunc Cell) Dim {
	xPos := -g.Horizontal.Offset
	for col := g.Horizontal.First; col <= g.Horizontal.Last; col++ {
		trans := op.Offset(Pt(xPos, 0)).Push(gtx.Ops)
		c := gtx
		c.Constraints = Exact(Pt(dimensioner(Horizontal, col, gtx.Constraints.Max.X), rowHeight))
...
func (g *Grid) Update(gtx Gx, rows, cols int, dim Dimensioner) {
	rowHeight := dim(Vertical, 0, gtx.Constraints.Max.Y)
	...
	g.Horizontal.update(gtx, Horizontal, cols, gtx.Constraints.Max.X, dim)

	lockedHeight := rowHeight * g.LockedRows
	g.Vertical.update(gtx, Vertical, rows-g.LockedRows, gtx.Constraints.Max.Y-lockedHeight, dim)
}
...
func (g *Grid) Layout(gtx Gx, rows, cols int, dimensioner Dimensioner, cellFunc Cell) Dim {
	...
	for row := 0; row < g.LockedRows && row < rows; row++ {
		rowHeight := dimensioner(Vertical, row, gtx.Constraints.Max.Y)
		lockedHeight += rowHeight

		offset := op.Offset(Pt(0, yOffset)).Push(gtx.Ops)
	...
	return Dim{Size: listDims, Baseline: 0}
}
