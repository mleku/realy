// Package richtext provides rendering of text containing multiple fonts,
// styles, and levels of interactivity.
package richtext
...
func (i *InteractiveSpan) Update(g Gx) (Event, bool) {
	...
	if !i.longPressed && i.pressing && g.Now.Sub(i.pressStarted) > LongPressDuration {
		i.longPressed = true
...
func (i *InteractiveSpan) Layout(g Gx) Dim {
	...
	defer clip.Rect{Max: g.Constraints.Max}.Push(g.Ops).Pop()
...
func (t TextStyle) Layout(g Gx) Dim {
	...
	return text.Layout(g, func(g Gx, i int, _ Dim) {
		span := &t.Styles[i]
		if !span.Interactive {
			return
		}
