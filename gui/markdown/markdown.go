...
type Config struct {
	...
	H1Size, H2Size, H3Size, H4Size, H5Size, H6Size Sp
	// Defaults to black.
...
// CommitCurrent compiles the state of the Current field and appends it to
...
func (g *gioNodeRenderer) UpdateCurrentSize(sp Sp) {
	g.Current.Size = sp
}
...
// UpdateCurrentFont uses the provided font as a set of attributes to update. If
// any of those attributes are not their zero value, the current text's
// corresponding attribute will be updated to match. If the provided font is the
// zero value, the current font will be reset to the zero value as well.
...
func (r *Renderer) Render(src []byte) ([]richtext.SpanStyle, error) {
	...
	if r.Config.H6Size == 0 {
		r.Config.H6Size = Sp(math.Round(1.2 * float64(r.Config.DefaultSize)))
	}
	if r.Config.H5Size == 0 {
		r.Config.H5Size = Sp(math.Round(1.2 * float64(r.Config.H6Size)))
	}
	if r.Config.H4Size == 0 {
		r.Config.H4Size = Sp(math.Round(1.2 * float64(r.Config.H5Size)))
	}
	if r.Config.H3Size == 0 {
		r.Config.H3Size = Sp(math.Round(1.2 * float64(r.Config.H4Size)))
	}
	if r.Config.H2Size == 0 {
		r.Config.H2Size = Sp(math.Round(1.2 * float64(r.Config.H3Size)))
	}
	if r.Config.H1Size == 0 {
		r.Config.H1Size = Sp(math.Round(1.2 * float64(r.Config.H2Size)))
	}
