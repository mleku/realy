...
func TestStyledtextRegressions(t *testing.T) {
	...
	for _, tc := range []testcase{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var ops op.Ops
			g := app.NewContext(&ops, app.FrameEvent{
				Metric: unit.Metric{PxPerDp: 1, PxPerSp: 1},
				Size:   tc.space,
			})

			txt.Layout(g, func(gtx Gx, idx int, dims Dim) {})
		})
	}
}
...
func TestStyledtextNewlines(t *testing.T) {
	gtx := app.NewContext(new(op.Ops), app.FrameEvent{
		Metric: unit.Metric{PxPerDp: 1, PxPerSp: 1},
		Size:   Point{X: 40, Y: 1000},
	})
	gtx.Constraints.Min = image.Point{}
	shaper := text.NewShaper(text.NoSystemFonts(), text.WithCollection(gofont.Collection()))

	singleLineTxt := Text(shaper, SpanStyle{Size: 12, Content: "a"})
	singleLineDims := singleLineTxt.Layout(gtx, func(g Gx, idx int, dims Dim) {})
