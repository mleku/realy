...
type textIterator struct {
	...
	lineOff Point
	// padding is the space needed outside the bounds of the text to ensure no
...
func (it *textIterator) processGlyph(g text.Glyph, ok bool) (_ text.Glyph, visibleOrBefore bool) {
	logicalBounds := Rectangle{
		Min: Pt(g.X.Floor(), int(g.Y)-g.Ascent.Ceil()),
		Max: Pt((g.X + g.Advance).Ceil(), int(g.Y)+g.Descent.Ceil()),
	}
...
// to the heap.
func (it *textIterator) paintGlyph(g Gx, shaper *text.Shaper, glyph text.Glyph, line []text.Glyph) ([]text.Glyph, bool) {
	...
	if glyph.Flags&text.FlagLineBreak > 0 || cap(line)-len(line) == 0 || !visibleOrBefore {
		t := op.Offset(it.lineOff).Push(g.Ops)
		op := clip.Outline{Path: shaper.Shape(line)}.Op().Push(g.Ops)
