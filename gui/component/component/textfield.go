...
// TextFieldPage holds the state for a page demonstrating the features of
...
// NewTextFieldPage constructs a TextFieldPage with the provided router.
func NewTextFieldPage(router *Router) *TextFieldPage {
	return &TextFieldPage{
		Router: router,
	}
}

var _ Page = &TextFieldPage{}

func (p *TextFieldPage) Actions() []component.AppBarAction {
	return []component.AppBarAction{}
}

func (p *TextFieldPage) Overflow() []component.OverflowAction {
	return []component.OverflowAction{}
}

func (p *TextFieldPage) NavItem() component.NavItem {
...
func (p *TextFieldPage) Layout(gtx C, th *material.Theme) D {
	...
	return material.List(th, &p.List).Layout(gtx, 1, func(gtx C, _ int) D {
		return layout.Flex{
			...
			layout.Rigid(func(gtx C) D {
				return DefaultInset.Layout(gtx, material.Body2(th, "Responds to hover events.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return DefaultInset.Layout(gtx, material.Body2(th, "Label animates properly when you click to select the text field.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return DefaultInset.Layout(gtx, material.Body2(th, "Can have prefix and suffix elements.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
				if err := func() string {
			...
			layout.Rigid(func(gtx C) D {
				return DefaultInset.Layout(gtx, material.Body2(th, "Can be validated.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return DefaultInset.Layout(gtx, material.Body2(th, "Can have a character counter and help text.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return DefaultInset.Layout(gtx, material.Body2(th, "This text field implementation was contributed by Jack Mordaunt. Thanks Jack!").Layout)
			}),
		)
	})
}
