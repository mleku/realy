...
`/capabilities` is an unprotected endpoint, and works like `nip-11` but is more sensible and relates to the RESTful endpoints described here.
...
- `path` means the path string after the relay URL that invokes the protocol API method.
