...
The distinction is that most of these HTTP endpoints _require_ authentication, but may not limit access by identity.

A second distinction is that calls to these endpoints MUST have an `Accept` header with a recognised encoding.
...
Calling this with `GET` will return a JSON array containing arrays with the following format:
