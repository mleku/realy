...
//go:noescape
func sha256X16Avx512(digests *[512]byte, scratch *[512]byte, table *[512]uint64,
	mask []uint64,
	inputs [16]by)
...
func (d *Avx512Digest) Sum(in by) (result by) {
	...
	d.a512srv.blocksCh <- blockInput{uid: d.uid, msg: trail, final: true,
		sumCh: sumCh}
	d.result = <-sumCh
	d.final = true
	return append(in, d.result[:]...)
...
func blockAvx512(digests *[512]byte, input [16]by,
	mask []uint64) [16][Size]byte {
...
func getDigest(index no, state by) (sum [Size]byte) {
	for j := 0; j < 16; j += 2 {
		for i := index*4 + j*Size; i < index*4+(j+1)*Size; i += Size {
			binary.BigEndian.PutUint32(sum[j*2:],
				binary.LittleEndian.Uint32(state[i:i+4]))
		}
	}
	return
}
...
func (a512srv *Avx512Server) Process() {
	for {
		...
		case block := <-a512srv.blocksCh:
			...
			a512srv.totalIn++
			a512srv.lanes[index] = Avx512LaneInfo{uid: block.uid,
				block: block.msg}
			if block.final {
...
func (a512srv *Avx512Server) getDigests() *[512]byte {
	...
	for i, lane := range a512srv.lanes {
		...
		if ok {
			binary.BigEndian.PutUint32(digests[(i+0*16)*4:],
				binary.LittleEndian.Uint32(a[0:4]))
			binary.BigEndian.PutUint32(digests[(i+1*16)*4:],
				binary.LittleEndian.Uint32(a[4:8]))
