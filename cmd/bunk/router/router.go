...
func NewRouter() *R {
	...
	nav := component.NewNav("bunk", "nostr signer bunker")
...
func (r *R) Register(tag interface{}, p Page) {
	r.pages[tag] = p
	navItem := p.NavItem()
	navItem.Tag = tag
...
func (r *R) SwitchTo(tag interface{}) {
	p, ok := r.pages[tag]
	if !ok {
		return
	}
...
func (r *R) Layout(gtx layout.Context, th *material.Theme) layout.Dimensions {
